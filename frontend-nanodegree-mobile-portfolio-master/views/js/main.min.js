...
var adjectives="dark color whimsical shiny noisy apocalyptic insulting praise scientific".split(" "),nouns="animals everyday fantasy gross horror jewelry places scifi".split(" ");function generator(a,b){a=getAdj(a);b=getNoun(b);var d=parseInt(Math.random()*a.length),c=parseInt(Math.random()*b.length);return"The "+a[d].capitalize()+" "+b[c].capitalize()}
...
return a+=ingredientItemizer(selectRandomCrust())},pizzaElementGenerator=function(a){var b=document.createElement("div");var d=document.createElement("div");var c=document.createElement("img");var e=document.createElement("div");b.classList.add("randomPizzaContainer");b.style.width="33.33%";b.style.height="325px";b.id="pizza"+a;d.style.width="35%";c.src="images/pizza.svg";c.classList.add("img-responsive");d.appendChild(c);b.appendChild(d);e.style.width="65%";a=document.createElement("h4");a.innerHTML=
randomName();e.appendChild(a);a=document.createElement("ul");a.innerHTML=makeRandomPizza();e.appendChild(a);b.appendChild(e);return b},resizePizzas=function(a){window.performance.mark("mark_start_resize");a:switch(a){case "1":document.querySelector("#pizzaSize").innerHTML="Small";break a;case "2":document.querySelector("#pizzaSize").innerHTML="Medium";break a;case "3":document.querySelector("#pizzaSize").innerHTML="Large";break a;default:console.log("bug in changeSliderLabel")}for(var b=0;b<document.querySelectorAll(".randomPizzaContainer").length;b++){var d=
document.querySelectorAll(".randomPizzaContainer")[b];var c=a;var e=d.offsetWidth,d=document.querySelector("#randomPizzas").offsetWidth,e=e/d;a:{switch(c){case "1":c=.25;break a;case "2":c=.3333;break a;case "3":c=.5;break a;default:console.log("bug in sizeSwitcher")}c=void 0}c=(c-e)*d;c=document.querySelectorAll(".randomPizzaContainer")[b].offsetWidth+c+"px";document.querySelectorAll(".randomPizzaContainer")[b].style.width=c}window.performance.mark("mark_end_resize");window.performance.measure("measure_pizza_resize",
"mark_start_resize","mark_end_resize");a=window.performance.getEntriesByName("measure_pizza_resize");console.log("Time to resize pizzas: "+a[a.length-1].duration+"ms")};window.performance.mark("mark_start_generating");for(var i=2;100>i;i++){var pizzasDiv=document.getElementById("randomPizzas");pizzasDiv.appendChild(pizzaElementGenerator(i))}window.performance.mark("mark_end_generating");window.performance.measure("measure_pizza_generation","mark_start_generating","mark_end_generating");
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");for(var a=document.querySelectorAll(".mover"),b=0;b<a.length;b++)a[b].style.left=a[b].basicLeft+100*Math.sin(document.body.scrollTop/1250+b%5)+"px";window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");0===frame%10&&(a=window.performance.getEntriesByName("measure_frame_duration"),logAverageFrame(a))}window.addEventListener("scroll",updatePositions);
document.addEventListener("DOMContentLoaded",function(){for(var a=0;200>a;a++){var b=document.createElement("img");b.className="mover";b.src="images/pizza.svg";b.style.height="100px";b.style.width="73.333px";b.basicLeft=a%8*256;b.style.top=256*Math.floor(a/8)+"px";document.querySelector("#movingPizzas1").appendChild(b)}updatePositions()});
